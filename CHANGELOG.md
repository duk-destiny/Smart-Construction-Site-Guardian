...
- **低置信度 LLM 辅助研判（用户实测反馈驱动）**：复核 Agent 判定
  needs_review 时后台异步调 LLM 产出"辅助理解"意见（可能原因/现场复核要点/
  建议优先级三段式，≤200 字），落 agent_runs 证据链并在研判页以紫色意见卡
  展示（明确标注"仅辅助理解·不改变定级"）；异步执行不占主链路时延，
  LLM 不可用/异常静默降级留痕；实时链路保持纯规则（轻链路铁律不动）；
  :memory: 库环境降级为仅日志留痕；

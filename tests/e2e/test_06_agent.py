...
def main():
    ...
    with sync_playwright() as p:
        ...
        expect(page.get_by_text("多 Agent 研判", exact=True).first).to_be_visible(timeout=8000)
        check("空态提示", page.get_by_text("请从「统一上报 → 影像研判」发起任务").count() > 0)

        # ===== 2. 无效任务 → 重试入口 =====
        page.goto(BASE + "/agents/t_does_not_exist", wait_until="networkidle")
        try:
            expect(page.get_by_role("button", name="开始 / 重试多 Agent 研判").first).to_be_visible(timeout=8000)
        ...
        check("重试入口", page.get_by_role("button", name="开始 / 重试多 Agent 研判").count() > 0)
        check("历史证据链区块", page.get_by_text("Agent 运行证据链", exact=True).count() > 0)

        # ===== 4. 重新研判：轮询 UI → 结果回流 =====
        page.get_by_role("button", name="开始 / 重试多 Agent 研判").click()
        try:
